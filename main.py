...
def sort_pics_by_extension_and_date(dir_path):
    # create directories for each file extension
    sortedPath=dir_path
    for root, dirs, files in os.walk(dir_path):
        for file in files:
...
def sort_documents_by_extension_and_date(dir_path):
    ...
    for root, dirs, files in os.walk(dir_path):
        for file in files:
            if file.lower().endswith((".doc", ".docx", ".docm", ".dot", ".dotx", ".dotm", ".xls", ".xlsx", ".xlsm", ".xlsb", ".xlt", ".xltx", ".xltm", ".ppt", ".pptx", ".pptm", ".pot", ".potx", ".potm", ".ppa", ".ppam", ".pps", ".ppsx", ".ppsm", ".sldx", ".sldm", ".accdb", ".accdt", ".accdr", ".pub", 'pdf')):
                # get file extension
...
if __name__ == '__main__':
    if len(sys.argv)>=3:
        ...
        if command.lower() == "document":
            ...
            sort_pics_by_extension_and_date(dir_path)
            delete_empty_folders(dir_path)
    else:    
